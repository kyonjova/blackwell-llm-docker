...
- DS4 text sets `VLLM_ENABLE_STARTUP_PLAN=1`. The first start stores the
  measured KV budget under the JIT cache; later starts with the same vLLM
  configuration and device reuse it and skip the CUDA-graph memory estimate
  (about 45-50 s on TP2). A plan is refused when free GPU memory shrank. The
  key covers CLI settings, not environment variables: after changing an
  environment variable that affects GPU memory, start once with
  `-e VLLM_ENABLE_STARTUP_PLAN=0` or set `KV_CACHE_MEMORY_BYTES`.
