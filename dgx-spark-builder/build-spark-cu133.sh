...
for k in VLLM_PATCH_FILE VLLM_PATCH_SHA256 B12X_PATCH_FILE B12X_PATCH_SHA256 LMCACHE_PATCH_FILE LMCACHE_PATCH_SHA256 \
         VLLM_INTEGRATION_TREE B12X_INTEGRATION_TREE LMCACHE_INTEGRATION_TREE; do export "${k}=${!k:-}"; done
...
# --------------------------------------------------------------- patch toggles
for t in PATCH_GPU_ARCH PATCH_MAX_JOBS PATCH_EXLLAMAV3_AVX; do
...
if [[ "${PHASE}" == all || "${PHASE}" == overlay ]]; then
  ...
  for c in VLLM B12X LMCACHE; do for s in REPO REF COMMIT PATCH_FILE PATCH_SHA256 INTEGRATION_TREE; do v="${c}_${s}"; args+=(--build-arg "${v}=${!v:-}"); done; done
  for v in INSTANTTENSOR_REPO INSTANTTENSOR_COMMIT INSTANTTENSOR_VERSION INSTANTTENSOR_LIBAIO_REPO INSTANTTENSOR_LIBAIO_COMMIT INSTANTTENSOR_LIBAIO_TREE \
           EXLLAMAV3_REPO EXLLAMAV3_COMMIT DEEPGEMM_REPO DEEPGEMM_COMMIT TRITON_KERNELS_REPO TRITON_KERNELS_COMMIT; do args+=(--build-arg "${v}=${!v:-}"); done
  build "${args[@]}"
