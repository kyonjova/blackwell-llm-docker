...
# vllm-jovian wheel tags: the cu134 wheel-release CI tags every commit on
# dev/jovian-judgement, and compose_source's fetch auto-follows tags pointing
# at the fetched commit. The overlay runs `python tools/build_rust.py
# --release` whose bare setup() lets the setuptools_scm integration describe
# WITHOUT the --exclude that tools/build_rust.py's direct get_version() uses,
# so an unparseable beta tag at distance 0 raises "ValueError: Can't parse
# version from tag" (r39 first hit, 2026-09-16). Deleting the
# source-addressed tags restores the pre-publisher behavior; no-op for the
# b12x/LMCache components and invisible to the write-tree verification.
python3 - "${D_OVER}" "${PATCH_VLLM_WHEEL_TAGS}" <<'PYEOF'
import pathlib, sys
path, tog = pathlib.Path(sys.argv[1]), sys.argv[2]
text = path.read_text()
anchor = 'test "$(git -C "${destination}" rev-parse HEAD)" = "${commit}"; \\'
...
if tog == "off":
    ...
    print("PATCH_VLLM_WHEEL_TAGS(auto): compose_source commit-check line absent; "
