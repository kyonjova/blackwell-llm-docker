...
TOG_SCM = _tog("PATCH_SCM_PRETEND_VERSION")
...
# setuptools_scm pretend-version. tools/build_rust.py calls a bare setup()
# with no literal version, so setuptools_scm's infer_version entry point runs
# `git describe` against /opt/vllm. When the nearest tag is not PEP 440 --
# e.g. vllm-jovian-cu134-beta-<sha> (2026-09-13, on the r38 pin), where
# tag_regex strips the prefix with (?:[\w-]+-)? and hands the bare 40-hex sha
# to the version parser -- setuptools_scm 10.x / vcs-versioning >= 2.4 raise
# ValueError instead of falling back, and the RUN dies AFTER the ~4 minute
# vLLM compile has already succeeded. VLLM_VERSION_OVERRIDE does not cover
# this: vLLM's own setup.py reads it, build_rust.py does not. Pin the rust
# step to the same version string as the wheel, so neither setup() in this
# RUN inspects git at all.
text = path.read_text()
SCM_ANCHOR = 'export VLLM_VERSION_OVERRIDE="${VLLM_BUILD_VERSION}";'
SCM_ADD = ' export SETUPTOOLS_SCM_PRETEND_VERSION="${VLLM_BUILD_VERSION}";'
cnt = text.count(SCM_ANCHOR)
if TOG_SCM == "off":
    print("PATCH_SCM_PRETEND_VERSION=off: pretend-version export skipped",
          file=sys.stderr)
elif "SETUPTOOLS_SCM_PRETEND_VERSION" in text:
    print("PATCH_SCM_PRETEND_VERSION: already set upstream; skipping",
          file=sys.stderr)
elif cnt == 0 and TOG_SCM == "auto":
    # Auto-skip is only legitimate when the rust frontend is no longer built.
    # If build_rust.py is still invoked and the anchor moved, dying now costs
    # seconds; skipping costs the whole vLLM compile.
    assert "build_rust.py" not in text, (
        "PATCH_SCM_PRETEND_VERSION(auto): build_rust.py is still invoked but "
        "the VLLM_VERSION_OVERRIDE anchor did not match -- upstream changed "
        "shape; inspect the vllm-build RUN")
    print("PATCH_SCM_PRETEND_VERSION(auto): rust frontend no longer built; "
          "skipping", file=sys.stderr)
else:
    assert cnt == 1, f"VLLM_VERSION_OVERRIDE anchor found {cnt} times"
    path.write_text(text.replace(SCM_ANCHOR, SCM_ANCHOR + SCM_ADD))
    print("injected SETUPTOOLS_SCM_PRETEND_VERSION export "
          "before the vLLM build", file=sys.stderr)
PYEOF
