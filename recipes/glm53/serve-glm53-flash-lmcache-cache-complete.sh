...
if [[ ${transfer_mode} == engine_driven && -z ${shm_name} ]]; then
  ...
  # so including it also gives each concurrently runnable sidecar a distinct
  # POSIX shared-memory arena.
...
case "${lmcache_kv_cache_dtype}" in
  ...
  *)
    printf 'LMCACHE_KV_CACHE_DTYPE must be fp8_ds_mla or nvfp4_ds_mla; got %s\n' \
...
case "${vllm_kv_cache_dtype}" in
  fp8 | nvfp4_ds_mla) ;;
  *)
    printf 'LMCACHE_VLLM_KV_CACHE_DTYPE must be fp8 or nvfp4_ds_mla; got %s\n' \
      "${vllm_kv_cache_dtype}" >&2
    exit 2
    ;;
esac
if [[ ${lmcache_kv_cache_dtype} == fp8_ds_mla && \
  ${vllm_kv_cache_dtype} != fp8 ]]; then
  printf 'fp8_ds_mla storage requires vLLM --kv-cache-dtype fp8\n' >&2
  exit 2
fi
if [[ ${lmcache_kv_cache_dtype} == nvfp4_ds_mla && \
  ${vllm_kv_cache_dtype} != nvfp4_ds_mla ]]; then
...
if [[ ${transfer_mode} == engine_driven ]]; then
  # Engine-driven SHM advertises direct views into the complete L1 arena.
  # Lazy allocation cannot expose a stable pool address or capacity, so an
  # explicit SHM name requires a preallocated L1 arena.
  lmcache_server+=(--no-l1-use-lazy --shm-name "${shm_name}")
else
  # Lazy L1 keeps the default pickle and CUDA-IPC transports bounded by the
  # data actually stored instead of reserving the complete configured arena.
  lmcache_server+=(--l1-use-lazy)
fi

if [[ -n ${checkpoint_identity} ]]; then
  if [[ ${transfer_mode} != engine_driven ]]; then
    printf 'Semantic checkpoints require engine-driven SHM transport\n' >&2
    exit 2
  fi
  jq -e 'type == "object"' <<< "${checkpoint_identity}" >/dev/null
  if [[ -n ${LMCACHE_CHECKPOINT_INDEX_PATH:-} ]]; then
    mkdir -p "$(dirname "${LMCACHE_CHECKPOINT_INDEX_PATH}")"
    lmcache_server+=(--checkpoint-index-path "${LMCACHE_CHECKPOINT_INDEX_PATH}")
  fi
fi
...
if [[ ${transfer_mode} != engine_driven ]]; then
  readonly interposer=/opt/lmcache/lib/liblmcache_cumem_shareable.so
  if [[ ! -s ${interposer} ]]; then
    printf 'LMCache cuMem transfer requires a nonempty interposer at %s\n' \
      "${interposer}" >&2
    exit 2
  fi
  if ! nm -D "${interposer}" | awk '$3 == "cuMemCreate" { found=1 } END { exit !found }'; then
    printf 'LMCache cuMem interposer does not export cuMemCreate: %s\n' \
      "${interposer}" >&2
    exit 2
  fi
...
# The CPU-only engine-driven sidecar has no CUDA context on serving GPUs. The
# 0.950 profile leaves measured long-prefill headroom while allocating more KV
# capacity than the direct-transfer profile; retain an explicit operator
...
if [[ ${transfer_mode} == engine_driven ]]; then
  # The standalone cache server manages CPU and filesystem storage only. GPU
  # gather and scatter operations execute in the existing vLLM worker
  # processes, so creating an additional CUDA context in the sidecar consumes
  # device memory without providing a transfer service.
  ...
  for assignment in "${lmcache_server_env_args[@]}"; do
    if [[ ! ${assignment} =~ ^[A-Za-z_][A-Za-z0-9_]*=.*$ ]]; then
      printf 'LMCACHE_SERVER_ENV contains an invalid assignment: %s\n' \
        "${assignment}" >&2
      exit 2
    fi
  done
  lmcache_server_command=(
    env "${lmcache_server_env_args[@]}" "${lmcache_server[@]}"
  )
fi
...
retention_interval=${chunk_size}
if [[ -n ${checkpoint_identity} ]]; then
  connector_config=$(jq -c --argjson identity "${checkpoint_identity}" '
...
# Independent aligned objects require recurrent state at every object boundary.
# Atomic semantic bundles instead carry their own exact target/draft endpoint;
# they must not force additional periodic recurrent checkpoints.
vllm_extra_args+=(
  --prefix-cache-retention-interval "${retention_interval}"
